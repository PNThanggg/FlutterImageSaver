...
class AppDialog {
  static Future<T?> show<T>({
    ...
    List<DialogAction> options = const [
...
class DialogAction<T> {
  ...
  const DialogAction(
      {this.text, this.type = ActionType.positive, this.child, this.actionValue, this.onPressed});
}
...
class HDialogWidget<T> extends StatelessWidget {
  ...
  AlertDialog createAndroidDialog<T>({
    ...
    final dialog = AlertDialog(
      ...
      content: contentWidget != null && content == null ? contentWidget : Text(content ?? ''),
      actions: actions,
    );
    return dialog;
  }
  ...
  Widget createIOSDialog<T>(
    ...
    final actions = options.map((option) {
      return CupertinoButton(
        ...
        child: option.child ??
            Text(option.text ?? '', style: TextStyle(color: _getActionColor(option.type))),
      );
    }).toList();
  ...
  Widget createIOSSheetDialog<T>({
    ...
    final cancelButton = bottomSheetCancel == null
        ...
        : CupertinoActionSheetAction(
            onPressed: () {
              ...
              if (bottomSheetCancel.onPressed != null) bottomSheetCancel.onPressed!();
            },
    ...
    return CupertinoActionSheet(
      title: title == null ? null : Text(title),
      message: contentWidget != null && content == null ? contentWidget : Text(content ?? ''),
      actions: actions,
